...
class EditUserWidget(QtGui.QWidget, Ui_EditUserWidget):
    ...
    def searchListWidget(self):
        search_text=self.filtergroups.text()
        count_list=self.listGroups.count() 
        for i in range(count_list):
           # self.listGroups.takeItem()
            if self.listGroups.item(i).text().indexOf(search_text) == -1 :
    ...
    def searchTreeListWidget(self):
        pass
"""    
        search=self.filterAuthorizations.text()
       # QTreeWidgetItem ıtem = 0
        item = 0
        if (search.isEmpty()):
           # for (int i=0 ; i<treeAuthorizations.topLevelItemCount(); i++)
           for i in range(treeAuthorizations.topLevelItemCount()):
               item = treeAuthorizations.topLevelItem(i)
               if(!(item)):
                   pass
               showItems(item)
        else :
            for i in range(treeAuthorizations.topLevelItemCount()):
                 item =treeAuthorizations.topLevelItemCount())
                 if(!item):
                     pass
                 filterItems(item)
    def showItems(self,QTreeWidget item):
        ...
        for i in range(item.childCount):
            ...
            if(!child):
                pass
            showItems(child
    def filterItems(QTreeWidget item,QString search):
        bool result
        if(item.childCount()):
           result =  ! item.text(0).contains(search, Qt.CaseSensitive)
           if(!result):
              showItems(item)
              return result
           result=True
           ...
           for i in range(item.childCount()):
              ...
              if(!child):
                   pass
              result=self.filterItems(child,search)
       else :
          result = !(item.text(0).contains(search,Qt.CaseSensitive)
       item.setHidden(False)
       return result 
