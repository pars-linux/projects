...
<channel>
	...
	<item>
		...
		<category><?=get_category_name($posts[$i]['cat_id'])?></category>
		<guid isPermaLink="true"><?=$config['core']['webaddress']?>?id=<?=$posts[$i]['id']?></guid>
		<description><?=htmlspecialchars(strip_tags($posts[$i]['text']), ENT_QUOTES)?></description>
		<content:encoded><?=htmlspecialchars($posts[$i]['text'], ENT_QUOTES)?></content:encoded>
