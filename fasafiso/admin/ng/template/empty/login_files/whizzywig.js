var whizzywig_version = 'Whizzywig v47'; //table width!; double smartquotes!; xml in spans
//Copyright © 2005, John Goodman - john.goodman(at)unverse.net  *date 051012
...
function makeWhizzyWig(txtArea, controls){ // make a WhizzyWig from the textarea
 if ((navigator.userAgent.indexOf('Safari') != -1 ) || !document.getElementById || !document.designMode ) {//no designMode
  alert("Whizzywig "+t("sizin tarayiciniza uygun değil !!"));
  return;
 }
 idTa = txtArea;
 ...
 w('<style type="text/css">.selector{margin:0 2px 0px 0;} .btnImage {cursor:pointer; cursor:hand; margin-top:3px; border:2px outset; border-color: #ccc;}  .ctrl {background:ButtonFace; border:1px outset ButtonShadow; padding:1px;width:'+taWidth+'} #sourceTa{color:#060;font:mono small;}</style>');
 var sels = 'formatblock fontname fontsize';
 var buts = ' bold italic underline | left center right | number bullet indent outdent | undo redo  | color hilite rule | link image table | clean html';
 ...
 w(fGo('LINK'));
 if (linkBrowse) w('<input type="button" onclick=doWin("'+linkBrowse+'"); value="'+t("Gözat")+'"> ');
 w(t('Adres (URL)')+': <input type="text" id="lf_url" size="60" value="http://"><br><input type="checkbox" id="lf_new">'+t("Bağlantıyı yeni pencerede aç")+fNo(t("Tamam"),"insertLink()")+'</div>');//LINK_FORM end
 w(fGo('IMAGE'));
 if (imageBrowse) w('<input type="button" onclick=doWin("'+imageBrowse+'"); value="'+t("Gözat")+'"> ');
 w(t('Resim Adresi (URL)')+': <input type="text" id="if_url" size="50" value="http://"> <label title='+t("to display if image unavailable")+'><br>'+t("Alternatif metin")+':<input id="if_alt" type="text" size="50"></label><br>'+t("Konum")+':<select id="if_side"><option value="none">'+t("normal")+'</option><option value="left">'+t("Sola")+'</option><option value="right">'+t("Sağa")+'</option></select> '+t("Kenar")+':<input type="text" id="if_border" size="20" value="none" title="'+t("number or CSS e.g. 3px maroon outset")+'"> '+t("Margin")+':<input type="text" id="if_margin" size="20" value="0" title="'+t("number or CSS e.g. 5px 1em")+'">'+fNo(t("Resim Ekle"),"insertImage()")+'</div>');//IMAGE_FORM end
 w(fGo('TABLE')+t("Satır")+':<input type="text" id="tf_rows" size="2" value="3"> <select id="tf_head"><option value="0">'+t("Satır başlık içermez")+'</option><option value="1">'+t("Satır başlık içerir")+'</option></select> '+t("Sütun")+':<input type="text" id="tf_cols" size="2" value="3"> '+t("Kenar Kalınlığı")+':<input type="text" id="tf_border" size="2" value="1"> '+fNo(t("Tablo Ekle"),"makeTable()")+'</div>');//TABLE_FORM end
 w(fGo('COLOR')+'<input type="hidden" id="cf_cmd"><div style="background:#000;padding:1px;height:22px;width:125px;float:left"><div id="cPrvw" style="background-color:red; height:100%; width:100%"></div></div> <input type=text id="cf_color" value="red" size=17 onpaste=vC(value) onblur=vC(value)> <button type="button" onmouseover=vC() onclick=sC()>'+t("Tamam")+'</button>  <button type="button" onclick="hideDialogs();">'+t("İptal")+' </button><br clear=all> <table border=0 cellspacing=1 cellpadding=0 width=480 bgcolor="#000000">');
 ...
 w('</table></div>'); //end color table,COLOR_FORM
  w('</div>'); //controls end
  w('<div id="sourceBox" class="ctrl" style="width:800px;height:'+taHeight+';position:absolute;display:none;"><button id="showWYSIWYG" type="button" onclick="showDesign();">'+t("HTML Kodu Gizle")+'</button><br> <textarea id="sourceTa" style="width:800px;height:95%">'+taContent+'</textarea></div>');
...
function makeButton(button){  // assemble the button requested
 ...
 if (!buttonPath || buttonPath == "textbuttons" || buttonPath == "" )
   ...
   var butHTML = '<img class="btnImage" src="'+buttonPath+button+'.png"  onMouseDown="selDown(this)" onMouseUp="selUp(this)" alt="'+t(ucBut)+'" title="'+t(ucBut)+'" onClick=makeSo("'+button+'")>';
 w(butHTML);
}
...
function fNo(txt,go){ //form do it/cancel buttons
 return ' <input type="button" onclick="'+go+'" value="'+txt+'"> <input type="button" onclick="hideDialogs();" value='+t("İptal")+'>';
}
...
function makeSelect(select){ // assemble the <select> requested
 ...
 var options = [t("Stil Seçin")+":", t("Paragraf"), t(h)+" 1 ", t(h)+" 2 ", t(h)+" 3 ", t(h)+" 4 ", t(h)+" 5 ", t(h)+" 6", t("Adres"), t("Büyük"), t("Küçük"), t("Kod <pre>")];
 ...
 var options = [t("Yazı Tipi")+":", "Arial", "Comic", "Courier New", "Georgia", "Times New Roman", "Verdana"];
 } else if (select == 'fontsize') {
  var values = ["3", "1", "2", "3", "4", "5", "6", "7"];
  var options = [t("Yazı Boyutu")+":", "1 "+t("Küçük"), "2", "3", "4", "5", "6", "7 "+t("Büyük")];
 } else { return }
 ...
 if ((rows > 0) && (cols > 0)) {
  ...
  for (var i=1; i <= rows; i++) {
   ...
   for (var j=1; j <= cols; j++) {
    if (i==1) {
     if (head=="1") table += "<th>Başlık"+j+"</th>"; //Title1 Title2 etc.
     else table += "<td>"+j+"</td>";
    }
...
function cleanUp(){  //tidy up crud inserted by Micro$oft Orifice
 ...
 iHTML = iHTML.replace(/[‘’]/g, "'"); //single smartquotes ‘’ 
 iHTML = iHTML.replace(/[“”]/g, '"'); //double smartquotes “”
 iHTML = iHTML.replace(/align=justify/gi, ""); //justify sends some browsers mad
 iHTML = iHTML.replace(/<(TABLE|TD)(.*)WIDTH[^A-Za-z>]*/gi, "<$1$2"); //no fixed width tables
...
function insHTML(html) { // insert arbitrary HTML at current selection
 whereAmI();
 if (!html) html = prompt("Eklemek üzere HTML kodu giriniz:", "");
 if (!html) return;
 if (document.all) { //IE
...
function textSel() {
	if (sel != "") return true; 
	else {alert(t("Öncelikle metin seçmelisiniz !")); return false;}
}
