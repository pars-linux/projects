...
class WriteXmlMixin:
    def write_xml(self, outfile):
        ...
        handler = saxutils.XMLGenerator(outfile, "utf-8")
...
class Image:
    ...
    def publish(self, handler):
        ...
        if isinstance(height, int):
            height = IntElement("width", height)
...
class Guid:
    ...
    Defaults to being a permalink.  Should it?  For that matter,
    the default when given a string is that it *isn't* a permalink.
    Should these two be swapped?  Are most guids permalinks?
    """
...
class Enclosure:
    ...
    def __init__(self, url, length, type):
        ...
        self.type = type
    def publish(self, handler):
        _element(handler, "enclosure", None, self.__dict__)
